<div class="card card-info">
    ...
    <div class="card-body">
            @if ($asignacionmovil->resource->count() > 0)
                <table class="table">
                    <thead>
                        <tr>
                            <th>Nombre</th>
                    ...
                    <tbody>
                        @foreach ($asignacionmovil->resource as $resource)
                                <tr>
                                    <th>{{ $resource->name }}</th>
                                    <td>{{ $resource->comentario }}</td>
                                    <td><i class="cursor-pointer fas fa-trash" wire:click='destroy({{ $resource->id }})'></i>
                                        <a href="{{ asset('storage/'.$resource->url) }}" target="_blanck"><i class="cursor-pointer fas fa-download"></i></a>
                                    </td>
                                </tr>
                        @endforeach
                    </tbody>
